...
function trimCSS(attrs, end, rerun, i=1, matches=new Set, comments=[], css='', ruleEnd, used='', rkeys, files, file, vw_breaks, styles, fn, endDump={}, dump={}) {
    ...
    fn=()=>{
      ...
      let _canAdd=!0, canAdd, at_rule, index=0, each, len=styles.length; canAdd=!0, each = styles.charAt(index); index<len;
      
      rerun=(canAdd=!0, each)=>{
      /** for loop used to boost the speed of the trimming algorithm, it is in bytes per run and defaults to 1 */
        for(let jump=0, boost=50000; jump<boost&&(each = styles.charAt(index)); jump++) {
           ...
           * in the comments which will lead to unclosed comments and parsing issues with the stylesheet
           */
          ...
          if(_canAdd&&each==='@') {
            ...
            temp=loop(styles, {from:index, cb:(s,f,t,r)=>{
               ...
               * is encountered once, this loop then continues looping over the keyframe styles until its end and checked by atRuleEnd
               */
              ...
              /** The control statement below gets @-rules that do not start with two curly braces like @charsest,  @import, @font-face up until where a semicolon ends in them.
               */
              else if(s[f]===';') {add=f; return true}

            }}),
            /**proceed to store block level @-rules, which are gotten from the last else if statement above,  till their closing curly brace
             * temp[0].match('@font-face') below maybe replaced with /@(font-face|property)/.test(temp[0]) to match other block level @-rules
            */
             ...
             * Dump uniquely stores media at rules based on their breakpoints, these are all added together to the end of the matched rules
            */
              ? /*add @import rules to the start of the used stylesheet*/(used+=res+added+added+(res.match('@import')?(canAdd=0, ';'):''), index=add) : (dump[at_rule]||=res+'{', index=add, css+=res);
          }

          //update 'each' for when index gets increased to 'jump' over the @-rules matched above
          canAdd&&(css+=each=styles.charAt(index));
          
          /** consider only class or id selectors. The regex test is to prevent matches like .5px in 0.5px.
          ...
          if(/\.|#/g.test(each)&&!/[0-9]/.test(styles.charAt(index+1))) attrs.forEach((attr, to='')=>{
            ...
            */
            if(attr===to[0]&&!/[0-9A-Za-z_-]/.test(styles.charAt(to[1]+1))) {
              ...
              : used+=res),
              index=forward[1],
              /** to remove repetitive class or id delimeters - # or . */
...
/** simple algorithm to check for the start of CSS comments and store the strings up till the end of the terminating string for the comment */
const storeComments=(i, s, arr)=>(loop(s, {from:i, cb:(s,f,t,r)=>s.charAt(f++)+s.charAt(f)==='*/'&&(i=f, arr.push(r[0]+'*/'), !0) }), i),

/** looks ahead about twice or thrice to know when a closing curly brace has another after it before an opening one
 * The condition for the loop terminates when a whitespace is encountered
 */
 ...
 * It is passed these arguments: str, from, to, result; the returned array to where loop is invoked. 
 * Back makes the loop work backwards until terminated by 'to', (has=str.charAt(from))===undefined or the returned value of cb
 * 
 * @returns an array [added strings, incremented index]
 */
