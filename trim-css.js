...
// require.main===module&&asScript();

function trimCSS(attrs, files, done, outDir, i, end, rerun, matches=new Set, comments=[], css='', ruleEnd, used='', generic='', warned, rkeys, file, vw_breaks, styles, fn, endDump={}, dump={}) {
...
// ),
	...
	warned = false;
      ...
      styles = files
      // fs.readFileSync(file=files[i++]).toString()
      +'\n';
...
Not to worry the arguments you provided before are temporary persisted on the disk and will be used.\n`+'-'.repeat(30))
      ...
      if(index===len) console.timeEnd('::TRIMMED:: '+file+' in:'), end(); /* called fn again to concurrently trim each .css file */ //i<files.length?fn():console.log(`\n::TRIMMED:: ${i} CSS files(s)\n`);
