let loop =  both.loop;
function trimCSS(attrs, progress, done, at, frameId, recon, end, callback, atRules=['import', 'keyframes', 'charset', 'font-face', 'property'], i=0, matches=new Set, comments=[], css='', ruleEnd, used='', rkeys, files, file, vw_breaks, styles, fn, endDump={}, dump={}) {
    ...
    fn=()=>{
      used='', css='', styles = file=files[at],
    //   all_attrs
      [attrs].forEach((attrs, attrs_index)=>{
        attrs = [...attrs],
        css='', /** clear the unused css to avoid redundancy only when treating attrs as an array of arrays of classes*/
        console.log('::DONE MATCHING.TRIMMING ATTRS::', attrs.length, attrs_index)
        
        // for(
            let canAdd, at_rule, keepIndex=0, index=2000, each, len=styles.length; canAdd=!0, each = styles.charAt(index); index<len;
            // );
        // setImmediate(
            callback=(canAdd=!0, each)=>{
          each = styles.charAt(index),
          progress([used, css], keepIndex, len, frameId),
          ...
          keepIndex = index++, trimCSS.reset=_=>index=len;

          if(index>len-1) cancelAnimationFrame(frameId), recon, done(keepIndex, used, frameId)/*end()*/;
